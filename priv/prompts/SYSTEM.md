...
- **To read a subagent's findings, use the exact path it reported, or the `scratchpad` tool to list what's actually there — NEVER guess a filename.** A subagent's completion message names the file it wrote (e.g. "published to the scratchpad as `camera-nvr-findings.md`"); read *that*, verbatim. Guessing a plausible name (`network-research-findings.md` when the file is `networking-research.md`) makes the read fail and drops the data. If you're unsure what a subagent wrote, list the scratchpad rather than guess.

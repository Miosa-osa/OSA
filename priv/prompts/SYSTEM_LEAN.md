...
1. **PLAN first.** 3+ steps → write it with `task_write` before touching code. One task `in_progress` at a time, updated as you finish each, never batched. Skip it for straightforward work — a single-step plan is pure latency.

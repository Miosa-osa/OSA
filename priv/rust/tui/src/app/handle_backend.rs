        ...
        match event {
            ...
            BackendEvent::ToolCallEnd {
                ...
                if orphan {
                    ...
                    self.collapse
                        .add_with_hooks(&kind, &args, success, call_hooks);
            ...
            BackendEvent::ToolsLoaded(result) => match result {
                Ok(tools) => {
                    ...
                    self.chat.set_welcome_info(&provider, &model, tools.len());
            ...
            BackendEvent::AccountLoginUpdate(result) => {
                match result {
                    Ok(session) => {
                        ...
                        if let Some(
                            crate::dialogs::model_picker::ModelPickerAction::SaveKeyAndSwitch {
                                provider,
                                runtime_provider,
                                api_key,
                                model,
            ...
            BackendEvent::BackgroundAgentCompleted {
                ...
                self.agents
                    .agent_completed(&agent_id, tool_uses, total_tokens, panel_summary);
            ...
            // Purely a roster update: it changes what the row SAYS, not what the
            // transcript contains. A phase is routine (every subagent passes
            // through several), so pushing a chat line or a toast per transition
            // would bury the actual work under lifecycle noise.
            BackendEvent::BackgroundAgentPhase {
                agent_id,
                display_name,
                phase,
                detail,
            ...
            BackendEvent::HookRun {
                ...
                debug!(
                    "Hook {} on {}: {} ({}ms)",
                    hook_name, hook_event, outcome, duration_ms
                );
            }
            ...
            BackendEvent::CompactionProgress {
                chunk_index,
                chunk_total,
            } => {
                // Measured work only — see `activity::progress_bar`.
                self.activity.set_details(
                    Some(crate::components::activity::progress_bar(
                        chunk_index,
                        chunk_total,
                    )),
                    1,
            ...
            BackendEvent::CompactionCompleted {
                ...
                // Two different mechanisms report through this one event, and
                // saying "messages folded" for both made the honest one read as
                // a lie. REPORTED LIVE on grok-4.6:
                //
                //     compacted 104.6k to 74.5k tokens ... 0 messages folded, 0 seconds
                //
                // That was the prune tier (`Compactor` `:micro_compact`, opencode's
                // `prune`): it erases stale tool OUTPUT in place, inside messages it
                // keeps, so it folds zero messages BY CONSTRUCTION and needs no
                // summariser call — hence 0s. The 30k it reclaimed was real. Only
                // the label was wrong, and the user reasonably read "0 messages, 0
                // seconds" as "nothing happened".
                //
                // The two mechanisms have disjoint signatures — a fold always
                // reduces the message count, a prune never does — so the phrasing
                // is derived from the same measured numbers as everything else
                // here rather than trusting a separate self-description.
                let what = if folded > 0 {
                    format!("{folded} messages folded")
                } else {
                    "tool output pruned in place, no messages folded".to_string()
                };

                // `fmt_elapsed(ms / 1000)` renders every sub-second run as "0s",
                // which is the other half of what read as "nothing happened". A
                // prune is genuinely instant; say that instead of claiming zero.
                let took = if duration_ms < 1000 {
                    "instant".to_string()
                } else {
                ...
                let line = format!(
                    "\u{2713} Compacted {} \u{2192} {} tokens ({}) \u{00B7} {}",
                    crate::components::status_bar::compact_tokens(tokens_before),

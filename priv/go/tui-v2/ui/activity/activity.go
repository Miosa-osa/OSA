...
// Model renders an elapsed timer and tool call feed.
type Model struct {
	active           bool
	startTime        time.Time
...
// New constructs a zero-value activity Model.
func New() Model {
	return Model{}
}
...
func (m Model) IsExpanded() bool { return m.expanded }

// ToolCount returns the total number of tool invocations so far.
func (m Model) ToolCount() int { return m.totalToolUses }

// InputTokens returns the accumulated input token count.
func (m Model) InputTokens() int { return m.inputTokens }

// OutputTokens returns the accumulated output token count.
func (m Model) OutputTokens() int { return m.outputTokens }

defmodule OptimalSystemAgent.Tools.Builtins.SecurityIntelTest do
  ...
  describe "hunter collectors" do
    ...
    test "anomaly dismiss without hop fails closed", %{ctx: ctx} do
      {:ok, rec} =
        run("anomaly_record", ctx, %{"target" => "https://example.com/x", "kind" => "http_500"})

      id = rec |> String.split() |> Enum.at(1)

      assert {:error, reason} = run("anomaly_dismiss", ctx, %{"key" => id})
      assert reason =~ "one hop"
    end

    test "validation_submit rejects confirmed without evidence", %{ctx: ctx} do
      assert {:error, reason} =
               run("validation_submit", ctx, %{
                 "finding" => %{
                   "verdict" => "confirmed",
                   "affected_asset" => "https://example.com/q",
                   "weakness_class" => "sqli"
                 }
               })

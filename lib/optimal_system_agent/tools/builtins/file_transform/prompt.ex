defmodule OptimalSystemAgent.Tools.Builtins.FileTransform.Prompt do
  ...
  def render(_opts \\ []) do
    ...
    Set `expect` on every mutating operation: it is the guard that makes an
    unread file safe to edit. Omitted it means "at least one" and zero matches is
    an error; given, it must match exactly.

    Answer questions about a file by running an operation over it, not by reading
    it — `count` and `assert_balanced` change nothing and return one line whether
    the file is 50 lines or 5,000. `dry_run: true` shows what an operation list
    ...
    `#{read_name}` is required, and do NOT `#{read_name}` afterwards: the
    observation already reports the lines and bytes changed and any post-edit
    validation failure.
    """
  end

defmodule OptimalSystemAgent.Tools.Builtins.TeamDelete.UI do
  ...
  def render(:tool_result, msg, _opts) when is_binary(msg) do
    %{kind: "team_delete_result", message: msg}
  end

  def render(:rejected, reason, _opts) when is_binary(reason) do
    %{kind: "team_delete_rejected", reason: reason}
  end

  def render(:error, msg, _opts) when is_binary(msg) do
    %{kind: "team_delete_error", message: msg}
  end

defmodule OptimalSystemAgent.Tools.Builtins.TaskOutput do
  ...
  The real implementation lives in:
      ...
      lib/optimal_system_agent/tools/builtins/task_output/
      ├── tool.ex        — `use OptimalSystemAgent.Tools.Behaviour`, callbacks
      ├── prompt.ex      — dynamic prompt with safe_ref cross-tool links
      ├── handler.ex     — validate / check_permissions / execute
      ├── ui.ex          — render/3 for the Rust TUI
      └── constants.ex   — tool_name/0

  This module exists so:
  1. The registry keeps the same atom `OptimalSystemAgent.Tools.Builtins.TaskOutput`.
  2. Existing test aliases (`alias ... TaskOutput`) continue to resolve.
  """

  @behaviour OptimalSystemAgent.Tools.Behaviour

  alias OptimalSystemAgent.Tools.Builtins.TaskOutput.Tool

defmodule OptimalSystemAgent.ContextRefs.Resolver do
    ...
    {blocks, _remaining} =
      Enum.reduce(refs, {[], budget}, fn ref, {acc, remaining} ->
        if remaining <= 0 do
          ...
          case resolve_one(ref, remaining) do
            {:ok, block} ->
  ...
  defp resolve_one({:file, path, range}, budget), do: Resolvers.File.resolve(path, range, budget)
  defp resolve_one({:diff, _}, budget), do: Resolvers.Diff.resolve(budget)
  defp resolve_one({:staged, _}, budget), do: Resolvers.Staged.resolve(budget)
  defp resolve_one({:git, n}, budget), do: Resolvers.GitLog.resolve(n, budget)
  defp resolve_one({:url, url}, budget), do: Resolvers.Url.resolve(url, budget)

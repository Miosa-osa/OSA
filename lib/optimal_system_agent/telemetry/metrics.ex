defmodule OptimalSystemAgent.Telemetry.Metrics do
  ...
  def handle_cast({:record_tool, name, duration_ms, success}, state) do
    entry = Map.get(state.tools, name, %{count: 0, success: 0, fail: 0, total_ms: 0, min_ms: nil, max_ms: 0, durations: []})
  ...
  def handle_cast({:record_provider, provider, duration_ms, success}, state) do
    entry = Map.get(state.providers, provider, %{count: 0, success: 0, fail: 0, total_ms: 0, min_ms: nil, max_ms: 0, durations: []})
  ...
  def handle_call(:snapshot, _from, state) do
    ...
    tools_with_p99 = Enum.map(state.tools, fn {name, entry} ->
    ...
    providers_with_p99 = Enum.map(state.providers, fn {name, entry} ->
      p99 = percentile(entry.durations, 99)
      avg = if entry.count > 0, do: round(entry.total_ms / entry.count), else: 0
      {name, Map.merge(entry, %{p99_ms: p99, avg_ms: avg}) |> Map.delete(:durations)}
    end) |> Map.new()

    {:reply, %{tools: tools_with_p99, providers: providers_with_p99, sessions: state.sessions}, state}
  end
